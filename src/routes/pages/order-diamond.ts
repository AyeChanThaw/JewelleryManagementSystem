...
class OrderDiamondRouter extends ExpressRouter {
  constructor() {
    super();

    this.route("/order-diamond").all(Permission.onLoad).get(this.getList);
    this.route("/order-diamond/entry").all(Permission.onLoad).get(this.getEntry).post(this.postEntry);
    this.route("/order-diamond/edit/:id").all(Permission.onLoad).get(this.getEdit).post(this.postEdit);
    this.route("/order-diamond/delete/:id").all(Permission.onLoad).post(this.postDelete);
    this.route("/order-diamond/preview/:id").all(Permission.onLoad).get(this.getDetail);
    this.route("/order-diamond-delete").all(Permission.onLoad).get(this.getDeleteList);
    this.route("/order-diamond-delete/recover/:id").all(Permission.onLoad).post(this.postRecover);
  }
  ...
  public getList(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    if (typeof (<any>req).jwtToken == "function") {
      (<any>req).jwtToken(jwtCredentialId)
        .then((result: string) => {
          params.token = result;
          res.render("dashboard/order-diamond", params);
        })
        .catch((err: any) => {
          next(err);
        });
  ...
  public getEntry(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    let params: any = { title: config.appname, user: req.user.username, postUrl: "/order-diamond/entry", params: {}, listUrl: "/order-diamond" };
    params = Permission.getMenuParams(params, req, res);
    const prefix = "ORD";
    let count = 0;
    ...
    RestApi.getDb("customer")
      ...
      .then((result) => {
        ...
        res.render("dashboard/order-diamond-entry", params);
      })
      .catch((err: any) => {
        next(err);
      });
    ...
    //       params.token = result;
    //       params.params.date = moment(new Date()).format("DD/MM/YYYY");
    //       res.render("dashboard/order-entry", params);
    //     })
    //     .catch((err: any) => {
    //       next(err);
    //     });
  ...
  public postEntry(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    RestApi.getDb("autogenerate").select("*").where("prefix", "ORD")
      .then((result) => {
        ...
        if (result[0].lastdate == datecode) {
          return RestApi.getDb("autogenerate").where("prefix", "ORD").update({ "currentvalue": count, "lastdate": datecode, "updateddate": date }, "id") as PromiseLike<any>;
        }
      })
      .then((result) => {
      ...
      .then((result) => {
        return RestApi.getDb("order_diamond").insert(data, "id");
      })
      .then((result) => {
  ...
  public getEdit(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    const postUrl = `/order-diamond/edit/${data.id}`;
    let params: any = { title: config.appname, user: req.user.username, postUrl: postUrl, listUrl: "/order-diamond" };
    params = Permission.getMenuParams(params, req, res);

    RestApi.getDb("order_diamond")
      .leftJoin("customer", "customer_id", "customer.id")
      .where({ "order_diamond.id": data.id })
      .select("order_diamond.*", "customer.code as customer_code", "customer.customer_name", "customer.phone")
      .then((result) => {
        ...
        return RestApi.getDb("order_payment").leftJoin("bank", "bank_id", "bank.id").where({"order_id": data.id, "type": "order_diamond"}).select("order_payment.*", "bank.bank_name", "bank.account");
      })
      .then((result) => {
        if (result.length > 0) {
  ...
  public postEdit(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    query.push(RestApi.getDb("daily_cash_in_showroom").where({"type_id": data.id, "status": "order_diamond"}).delete());
    payment_array.forEach((payment: any) => {
    ...
    cash_in = comfunc.fillDefaultFields(cash_in);
    query.push(RestApi.getDb("daily_cash_in_showroom").insert(cash_in, "id"));
    query.push(RestApi.getDb("daily_cash_showroom").where({"type_id": data.id, "type": "order_diamond"}).update({"cash_in": data.total_amount, "updateddate": date}));
    delete (data.payment_array);
    delete (data.customers);
    delete (data.customer_code);
    delete (data.customer_name);
    delete (data.phone);
    let db = RestApi.getDb("order_diamond");
    if (Utils.isEmpty(data.id)) {
  ...
  public postDelete(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    query.push(RestApi.getDb("daily_cash_showroom").where({"type_id": data.id, "type": "order_diamond"}).delete());
    query.push(RestApi.getDb("daily_cash_in_showroom").where({"type_id": data.id, "status": "order_diamond"}).delete());
    query.push(RestApi.getDb("daily_cash_payment").where({"cash_id": data.id, "type": "cash_in_showroom"}).delete());
    RestApi.getDb("sale_order")
      .where("order_id", data.id)
      .select()
      .then((result) => {
        sale_diamond = result;
        if (sale_diamond > 0) {
          ...
          return RestApi.getDb("order_payment").where("order_id", data.id).update({ "is_delete": 1 });
          // return RestApi.getDb("order_payment").where("order_id", data.id).delete();
        }
      })
      .then((result) => {
        return Promise.all(query);
      ...
      .then((result) => {
        return RestApi.getDb("order_diamond").where("id", data.id).update({ "is_active": 0, "is_delete": 1, "deleted_user_id": user_id, "deleted_date": Utils.toSqlDate(new Date()) });
      })
      .then((result) => {
        res.json({ "success": result });
      })
  ...
  public getDeleteList(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    if (typeof (<any>req).jwtToken == "function") {
      (<any>req).jwtToken(jwtCredentialId)
        .then((result: string) => {
          params.token = result;
          res.render("dashboard/order-diamond-delete", params);
        })
        .catch((err: any) => {
          next(err);
        });
  ...
  public postRecover(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    RestApi.getDb("order_diamond")
      ...
      .then((result) => {
        return RestApi.getDb("order_payment").where({"order_id": data.id, "type": "order_diamond"}).update({ "is_delete": 0 });
      })
      .then((result) => {
        return RestApi.getDb("order_diamond").where("id", data.id).update({ "is_active": 1, "is_delete": 0, "recover_user_id": user_id, "recover_date": Utils.toSqlDate(new Date()) });
      })
      .then((result) => {
        res.json({ "success": result });
      })
  ...
  public getDetail(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    const postUrl = `/order-diamond/preview/${data.id}`;
    let params: any = { title: config.appname, user: req.user.username, postUrl: postUrl, params: data };
    params = Permission.getMenuParams(params, req, res);

    RestApi.getDb("order_diamond")
      .leftJoin("customer", "order_diamond.customer_id", "customer.id")
      .leftJoin("sale_person", "order_diamond.sale_person_id", "sale_person.id")
      .where({ "order_diamond.id": data.id })
      .select("order_diamond.*", "customer.code as customer_code", "customer.customer_name", "customer.phone", "sale_person.sale_person_name")
      .then((result) => {
        result[0].date = Utils.toDisplayDate(result[0].date);
      ...
      .then((result) => {
        params.token = result;
        res.render("dashboard/order-diamond-preview", params);
      })
      .catch((err) => {
        console.log(`${err}`);
        next({ "error": err });
...
export default new OrderDiamondRouter();